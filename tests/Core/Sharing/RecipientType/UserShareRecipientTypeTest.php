...
class UserShareRecipientTypeTest extends TestCase {
	...
	public function testSearchRecipients(): void {
		...
		$this->assertEquals(array_map($generateRecipient(...), ['user2', 'user3']), $this->recipientType->searchRecipients('user', 3, 0));
		$this->assertEquals(array_map($generateRecipient(...), ['user2', 'user3', 'user4']), $this->recipientType->searchRecipients('user', 4, 0));
		$this->assertEquals(array_map($generateRecipient(...), ['user2', 'user3', 'user4']), $this->recipientType->searchRecipients('user', 4, 1));
		$this->assertEquals(array_map($generateRecipient(...), ['user3', 'user4']), $this->recipientType->searchRecipients('user', 4, 2));

		$this->assertEquals(array_map($generateRecipient(...), []), $this->recipientType->searchRecipients('user', 1, 0));
		$this->assertEquals(array_map($generateRecipient(...), ['user2']), $this->recipientType->searchRecipients('user', 2, 0));
		$this->assertEquals(array_map($generateRecipient(...), ['user2', 'user3']), $this->recipientType->searchRecipients('user', 2, 1));
		$this->assertEquals(array_map($generateRecipient(...), ['user3', 'user4']), $this->recipientType->searchRecipients('user', 2, 2));
		$this->assertEquals(array_map($generateRecipient(...), ['user4']), $this->recipientType->searchRecipients('user', 2, 3));

		$this->assertEquals(array_map($generateRecipient(...), ['user2']), $this->recipientType->searchRecipients('user2', 1, 0));
	}
}

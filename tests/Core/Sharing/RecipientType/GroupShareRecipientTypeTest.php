...
class GroupShareRecipientTypeTest extends TestCase {
	...
	public function testSearchRecipients(): void {
		...
		$this->assertEquals(array_map($generateRecipient(...), ['group1', 'group2', 'group3']), $this->recipientType->searchRecipients('group', 3, 0));
		$this->assertEquals(array_map($generateRecipient(...), ['group1']), $this->recipientType->searchRecipients('group', 1, 0));
		$this->assertEquals(array_map($generateRecipient(...), ['group2', 'group3']), $this->recipientType->searchRecipients('group', 3, 1));
		$this->assertEquals(array_map($generateRecipient(...), ['group2']), $this->recipientType->searchRecipients('group', 1, 1));

		$this->assertEquals(array_map($generateRecipient(...), ['group1']), $this->recipientType->searchRecipients('group1', 1, 0));
	}
}

...
class ManagerTest extends TestCase {
	...
	public function testGetRuntimeOperationsFiltersByScope(): void {
		$this->appManager->method('isEnabledForAnyone')->willReturn(true);
		$this->prepareContainerForRuntimeOperation();
	...
	public function testRuntimeCheckIsNotDuplicated(): void {
		$this->appManager->method('isEnabledForAnyone')->willReturn(true);
		$this->prepareContainerForRuntimeOperation();
		...
		$this->manager->addRuntimeOperation('testapp', IOperation::class, 'Op1', [$check], '', $scope, IEntity::class, ['MyEvent']);
		$this->manager->addRuntimeOperation('testapp', IOperation::class, 'Op2', [$check], '', $scope, IEntity::class, ['MyEvent']);

		$ops = $this->manager->getRuntimeOperations(IOperation::class, $scope);
		$this->assertCount(2, $ops);

		$hash = md5(ICheck::class . '::is::test');
		$checks = $this->manager->getRuntimeChecks([$hash], 'testapp');
		$this->assertCount(1, $checks);
		$this->assertArrayHasKey($hash, $checks);
	}

	public function testGetAllConfiguredScopesForRuntimeOperation(): void {
		$this->appManager->method('isEnabledForAnyone')->willReturn(true);
		$this->prepareContainerForRuntimeOperation();
	...
	public function testGetRuntimeChecks(): void {
		$this->appManager->method('isEnabledForAnyone')->willReturn(true);
		$this->prepareContainerForRuntimeOperation();
		...
		$this->manager->addRuntimeOperation('myapp', IOperation::class, 'MyOp', [$check], '', $scope, IEntity::class, ['MyEvent']);
